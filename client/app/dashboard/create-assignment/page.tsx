...
export default function CreateAssignmentPage() {
  ...
  const fetchAIContent = async (type: string, prompt: string) => {
    try {
      ...
      toast({
        ...
        description: "Something went wrong while contacting the AI backend.",
        variant: "destructive",
      })
      return ""
    }
  ...
  //       // const prompt = `${assignmentTitle}. ${description}. Learning Objectives: ${learningObjectives}`
  //       // const prompt = `Create a classroom ${output} on the topic: "${assignmentTitle}". Description: ${description}. Learning Objectives: ${learningObjectives}`;
  //       const prompt = `
  // You're an experienced educator. Please generate "${output}" content for a "${ASSIGNMENT_TYPE_INFO[selectedType]?.title}" titled "${assignmentTitle}".
  ...
  //       const aiOutput = await fetchAIContent(output, prompt)
  //       newContent[output] = aiOutput
  //     }
...
Learning Objectives: ${learningObjectives}
    `;
      ...
      try {
        ...
        const text = result.text; // ✅ No () needed — it's a property, not a function
        newContent[output] = text ?? "No content generated";
      } catch (error) {
        console.error("Error generating AI content:", error);
        newContent[output] = "Error generating content.";
      }
    }
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <Label
                      ...
                      className={`text-sm font-normal ${!connectedLMS.includes("Google Classroom") ? "text-muted-foreground" : ""
                        }`}
                    >
